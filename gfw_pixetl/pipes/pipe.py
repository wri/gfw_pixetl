...
from gfw_pixetl.parallelpipe import Pipeline, stage
...
class Pipe(ABC):
    ...
    def _build_pipe(self, tiles: List[Tile], workers: int) -> Pipeline:
        """Construct the processing pipeline for *tiles*.

        Subclasses override this to configure their memory-intensive
        stage with the supplied worker count.
        """
        raise NotImplementedError("Subclasses must implement _build_pipe()")
    ...
    def _process_pipe(
        ...
        This is the single-run pipeline entry point.
        """
